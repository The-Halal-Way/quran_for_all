...
class _HadithScreenState extends State<HadithAnNawawiView>
  ...
  final bool _showJumpDialog = false;
  ...
  void _openJumpDialog() {
    ...
    showDialog<void>(
      ...
      builder: (_) => _JumpDialog(
        isDark: _isDark,
        isBangla: _isBangla,
        cardBg: _cardBg,
        textMain: _textMain,
        textHint: _textHint,
        totalHadiths: _book!.hadiths.length,
        maxId: _book!.hadiths.map((h) => h.id).reduce(math.max),
        hadiths: _book!.hadiths,
        controller: controller,
        onJump: (id) {
          final idx = _book!.hadiths.indexWhere((h) => h.id == id);
          ...
          children: [
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 12),
                Text(
                  isBangla ? 'হাদিসে যান' : 'Jump to Hadith',
                  style: GoogleFonts.sora(
                    fontSize: 18,
                    fontWeight: FontWeight.w700,
                    color: textMain,
                ...
                GestureDetector(
                  ...
                  child: Icon(Icons.close_rounded, color: textHint, size: 20),
                ),
              ],
            ),
            ...
            TextField(
              ...
              decoration: InputDecoration(
                hintText: isBangla ? 'নম্বর লিখুন' : 'Enter number',
                ...
                border: OutlineInputBorder(
                  ...
                  borderSide: BorderSide(
                    color: MyColors.primaryLight.withOpacity(0.25),
                  ),
                ),
            ...
            // Quick access chips (first 10)
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: List.generate(
                math.min(10, hadiths.length),
                (i) {
                  final id = hadiths[i].id;
            ...
            GestureDetector(
              onTap: () {
                ...
                if (val == null) return;
                final idx = hadiths.indexWhere((h) => h.id == val);
                if (idx == -1) return;
                Navigator.of(context).pop();
                onJump(val);
              },
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(
                      color: MyColors.secondary.withOpacity(0.35),
                      blurRadius: 16,
                      offset: const Offset(0, 4),
                    ),
                  ],
...
/// Floating action-like jump button
